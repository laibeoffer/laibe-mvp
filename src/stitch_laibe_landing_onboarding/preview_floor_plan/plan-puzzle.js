(function () {
  ...
  const EXPORT_LIKE_OUTPUTS_ENABLED = false;
  const EXPORT_LIKE_DISABLED_MESSAGE = "Export-like outputs are disabled in this MVP preview: candidate mock only / no formal estimate / no Plancraft core production.";
  ...
  function handleActionClick(event) {
    ...
    if ((action === "export-draft" || action === "export-pc-spike") && !EXPORT_LIKE_OUTPUTS_ENABLED) {
      uiState.error = "";
      uiState.message = EXPORT_LIKE_DISABLED_MESSAGE;
      render();
      return;
    }
  ...
  function renderPcConverterReportCard() {
    ...
    return `
      <div class="status-card">
        ...
        ${validationNotice}
        <div class="inspector-actions" style="margin-top: 12px;">
          <button class="toolbar-btn" type="button" disabled data-mock-only="true" data-action="export-pc-spike" title="Candidate mock only; no Plancraft core production">.pc 測試版停用 / no production</button>
        </div>
        ${validationErrorList}
