...
const OWNER_MEMBERSHIP_KEYS = Object.freeze([
  "caseId",
  "role",
  "status",
  "userId",
...
export const SUPABASE_PUBLISHABLE_KEY =
  "sb_publishable_jU9DCme4-MBAc-kQuU2BQQ_gfmjzgLa";
export const SUPABASE_JS_MODULE_URL =
  "https://esm.sh/@supabase/supabase-js@2.112.3";
export const CANONICAL_ACCOUNT_ACCESS_URL =
  "http://127.0.0.1:4173/account/access/";
const PASSWORD_RECOVERY_REDIRECT_URL =
  `${CANONICAL_ACCOUNT_ACCESS_URL}?flow=password-recovery`;

export const OWNER_CONTRACT_INTENT_MESSAGE =
  "登入後預計前往甲方工作台的契約管理。";
...
export function accountAccessIntentMessage(location = globalThis.location) {
  try {
    ...
    return new URLSearchParams(search).get("intent") ===
        OWNER_CONTRACT_MANAGEMENT_INTENT
...
function isTrustedOwnerWorkspaceGrant(grant) {
  ...
  return ownValue(grant, "schemaVersion") ===
      "laibe.owner-workspace-runtime.v1" &&
...
export function validateRegister(values = {}) {
  ...
  if (!isPermittedRegistrationRole(role)) {
    errors.role = "請選擇你目前的使用角色。";
  }
  return errors;
}
...
export function validatePasswordUpdate(values = {}) {
  const errors = {};
  const password = typeof values.password === "string" ? values.password : "";
  const confirmPassword = typeof values.confirmPassword === "string"
...
export function isPasswordRecoveryReturn(location = globalThis.location) {
  ...
  const search = typeof location?.search === "string" ? location.search : "";
  if (new URLSearchParams(search).get("flow") === "password-recovery") {
    return true;
  }
...
export function createSupabaseAuthRuntime({
  ...
  if (!storage || typeof fetchImplementation !== "function") {
  ...
  function handleAuthStateChange(event, session) {
    const email = valueOf(session?.user?.email);
    if (!isAccessToken(session?.access_token) || !EMAIL_PATTERN.test(email)) {
      return;
    }
    authenticatedSession = session;
    if (
  ...
  async function signUpWithPassword(values = {}) {
    ...
    if (result?.error || !result?.data?.user) {
      throw new Error("ACCOUNT_SIGN_UP_UNAVAILABLE");
    }
  ...
  async function authenticatedFetch(endpoint, init = {}) {
    if (!ALLOWED_FUNCTION_ENDPOINTS.has(endpoint)) {
      throw new Error("FUNCTION_ENDPOINT_NOT_ALLOWED");
    }
    const session = await getSession();
    ...
    const response = await fetchImplementation(
      `${projectUrl}/functions/v1/${endpoint}`,
      {
        ...init,
        credentials: "omit",
        headers,
...
export function getSupabaseAuthRuntime(options) {
  ...
  if (!authRuntimePromise) {
    authRuntimePromise = Promise.resolve(
      importImplementation(SUPABASE_JS_MODULE_URL),
    ).then((module) => (
...
export async function resumeVendorSession({
  ...
  try {
    ...
    const response = await authRuntime.authenticatedFetch(
      "vendor-workspace-grant",
      { method: "GET" },
    );
...
export async function resumeAuthorizedSession({
  ...
  try {
    ...
    if (roleIntent === "invited-partner") {
      const vendorResult = await resumeVendorSession({
        authRuntime,
        navigate,
        routes,
        location,
    ...
    const ownerResponse = await authRuntime.authenticatedFetch(
      "owner-workspace-grant",
      { method: "GET" },
    );
    if (ownerResponse?.ok) {
...
function clearErrors(root, form, mode) {
  ...
  root.querySelectorAll(`[data-field-error^="${mode}-"]`).forEach((target) => {
    target.textContent = "";
  });
...
function showErrors(root, form, mode, errors) {
  ...
  for (const [fieldName, message] of Object.entries(errors)) {
    ...
    if (fieldName === "role") {
      root.querySelector(".role-binding")?.setAttribute("aria-invalid", "true");
    }
    const target = fieldName === "role"
      ? root.querySelector("[data-role-error]")
      : root.querySelector(
        `[data-field-error="${mode}-${errorFieldKey(fieldName)}"]`,
      );
...
function showAuthRuntimeUnavailable(root) {
  const message =
    "帳號與案件權限確認正在整理中，正式開放後可由此建立或登入帳號。";
...
function focusFirstError(root, form, errors) {
  ...
  const first = Object.keys(errors).find((name) =>
    name !== "role" && name !== "agree"
  );
...
async function handleSubmit(root, form, runtime) {
  ...
  const errors = (validators[mode] ?? (() => ({ form: "無法辨識這次操作。" })))(
    values,
  );
  ...
  if (Object.keys(errors).length) {
    setStatus(
      form,
      errors.role ? "請先選擇使用角色後再送出。" : "請確認標示欄位後再送出。",
      "error",
    );
  ...
  if (mode === "login") {
    ...
    try {
      ...
      await authRuntime.signInWithPassword(
        valueOf(values.email),
        values.password,
      );
      ...
      if (
        result.state === "OWNER_GRANTED" || result.state === "VENDOR_GRANTED"
      ) {
        setStatus(form, "登入成功，正在開啟工作台…", "success");
      } else {
        setStatus(
          form,
          "帳號已登入，但目前沒有可開啟的案件。請確認選擇的角色與案件資格。",
          "notice",
        );
      }
    } catch {
...
export function initAccountAccess(root = document, {
  ...
  root.querySelectorAll("[data-mode-tab]").forEach((button) => {
    button.addEventListener(
      "click",
      () => selectMode(root, button.dataset.modeTab),
    );
  });
  root.querySelectorAll("#acctType [data-type]").forEach((button) => {
  ...
  const roleIntent = new URLSearchParams(
  ...
  if (runtimePromise) {
    ...
    void Promise.resolve(runtimePromise)
      .then(async (authRuntime) => {
        ...
        try {
          stopRecoveryListener = authRuntime.onPasswordRecovery(
        ...
        if (recoveryReturn) {
          schedule(() => {
            ...
            const forgotForm = root.querySelector(
              '[data-account-form="forgot"]',
            );
            if (forgotForm) {
              setStatus(
                forgotForm,
          ...
          if (
            typeof authRuntime.resolvePasswordRecoverySession === "function"
          ) {
            try {
              confirmRecoveryAccount(
                await authRuntime.resolvePasswordRecoverySession(),
              );
            } catch {
        ...
        return resumeAuthorizedSession({
          authRuntime,
          navigate,
          routes,
          location,
      ...
      .then((result) => {
        ...
        if (loginForm) {
          ...
          setStatus(
            loginForm,
            "已完成登入，但目前沒有可開啟的案件。請確認登入 Email 與案件資格。",
            "notice",
          );
        }
      })
