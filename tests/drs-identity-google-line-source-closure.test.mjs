...
test("composition contains the admitted identity security subset without global status coupling", () => {
  assert.deepEqual(
    migrationName.length,
    1,
    "exactly one CLI migration is required",
  );
...
test("migration adds provider identity state without a competing authority graph", () => {
  ...
  for (
    ...
    assert.match(
      sql,
      new RegExp(required.replaceAll(".", "\\."), "iu"),
      required,
    );
  }
  assert.doesNotMatch(
...
test("OAuth state contract is digest-only, finite <=15m, encrypted, claimable and single-use", () => {
  ...
  assert.doesNotMatch(
    sql,
    /\braw_state\b|\bpkce_verifier\s+text|\bnonce\s+text/iu,
  );
});
...
test("security-definer producers are pinned and expose only exact service functions", () => {
  ...
  for (
    ...
    assert.match(
      sql,
      new RegExp(`revoke all on function integration\\.${name}\\(`, "iu"),
    );
    assert.match(
      sql,
      new RegExp(
        `grant execute on function integration\\.${name}\\([\\s\\S]+?to service_role`,
        "iu",
      ),
    );
  }
...
test("binding creation, revocation, login and terminal failure are audited once", () => {
  ...
  assert.doesNotMatch(
    sql,
    /release_identity_link_state_claim_v1|claimed_at\s*=\s*null|claim_token\s*=\s*null/iu,
  );
});
...
test("login completion is two-phase and returns only a validated browser continuation", () => {
  ...
  const finalizeCall = contracts.indexOf(
    "store.finalizeIdentityCallback",
    producerCall,
  );
...
test("production transports are injected and contain no live provider or Supabase fetch", () => {
  ...
  assert.doesNotMatch(
    combined,
    /SUPABASE_SERVICE_ROLE_KEY|service-role-secret|line[_-]?channel[_-]?secret|google[_-]?client[_-]?secret/iu,
  );
  assert.doesNotMatch(
    combined,
    /account\.line\.biz|localStorage|casework\.case_members/iu,
  );
...
test("endpoint source exposes method guards, fail-closed factories, and minimal session projection", () => {
  const endpoints = productionTs.filter((file) => file.endsWith("/index.ts"))
    .map(source).join("\n");
  const sessionContract =
    source("supabase/functions/_shared/drs-auth/contracts.ts") +
