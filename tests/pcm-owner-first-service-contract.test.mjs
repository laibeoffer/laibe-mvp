...
const FROZEN_CONTRACT_FILE_SHA256 =
  "16595e93277d31fda5c3fc8460445515ea9d1aa1b71af85cb7427c6d69741843";
...
test("T7 keeps the complete v0.3 contract-content file byte-for-byte frozen", async () => {
  ...
  assert.match(CONTRACT_SOURCE, /^# 萊比 LaiBE AI PCM 案件治理資訊服務契約$/m);
...
test("owner-first first fold states role, contract status, case status, responsibility and truthful actions", async () => {
  ...
  assert.match(firstFold, /契約狀態[\s\S]*v0\.3[\s\S]*法務審閱中/);
  ...
  assert.match(firstFold, /href="#full-contract"[^>]*>[\s\S]*開始閱讀完整契約/);
...
test("page explains the bounded draft-review-revision-confirmation sequence without claiming it is connected", async () => {
  ...
  assert.match(html, /PCM 公開提出審查意見/);
  assert.match(html, /乙方可不限次修訂/);
