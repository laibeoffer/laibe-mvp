...
const protectedExactFive = Object.freeze({
  "supabase/functions/_shared/drs-auth/drs-secure-session-runtime.ts":
    "859f71d94582fb79e8debbe325587ed346b092fd34e213864bec0bb8549dcae8",
  ...
  "supabase/tests/drs_secure_session_runtime_composition_w1.test.mjs":
    "348adc517067e30035d1ad61e0b6c5bc667f27459dc2314c5fdb591daeb5c7fa",
...
function parseNativePowerShellAst(ps) {
  ...
  assert.equal(ps.length > 0 && ps.length <= 1024 * 1024, true, "native parser input bound");
  ...
  try {
    ...
    assert.fail("native PowerShell Parser.ParseInput bridge emitted invalid JSON");
  }
  assert.deepEqual(parsed.parseErrors, [], "PowerShell source must parse without errors");
  return parsed;
}
...
function commandParameters(command) {
  ...
  for (let index = 1; index < command.elements.length; index += 1) {
    ...
    assert.equal(element.parameterArgument, null, `${command.name}: colon parameter arguments forbidden`);
    assert.equal(parameters.has(element.parameter), false, `${command.name}: duplicate -${element.parameter}`);
    let value = null;
  ...
  assert.deepEqual(positional, [], `${command.name}: positional or dynamic elements forbidden`);
  return parameters;
}
...
function powerShellReachability(ast) {
  ...
  for (const definition of ast.functions) {
    ...
    assert.equal(functions.has(key), false, `duplicate function definition: ${definition.name}`);
  ...
  for (const command of ast.commands) {
    ...
    if (command.name !== null) {
      ...
      if (!isFunction) {
        assert.equal(command.operator, "Unknown", `external call operator forbidden: ${command.name}`);
      }
    }
  }

  const unresolved = ast.commands.filter((command) => command.name === null);
  assert.equal(unresolved.length, 4, "exactly four unresolved terminateProcessTree invocations");
  ...
  assert.deepEqual(
    unresolvedShapes,
    [
      { owner: "Invoke-ClosedProcess", operator: "Ampersand", first: "$terminateProcessTree", text: "& $terminateProcessTree $false" },
      { owner: "Invoke-ClosedProcess", operator: "Ampersand", first: "$terminateProcessTree", text: "& $terminateProcessTree $false" },
      { owner: "Invoke-ClosedProcess", operator: "Ampersand", first: "$terminateProcessTree", text: "& $terminateProcessTree $false" },
      { owner: "Invoke-ClosedProcess", operator: "Ampersand", first: "$terminateProcessTree", text: "& $terminateProcessTree -exited $false" },
    ],
  ...
  return {
    ...
    commands: ast.commands.filter(
      (command) => command.owner === null || reachableFunctions.has(command.owner.toLowerCase()),
    ),
  };
}
...
function assertNativePowerShellSecurityContract(ps) {
  ...
  const closedCalls = reachable.filter((command) => command.name === "Invoke-ClosedProcess");
  const gitCalls = reachable.filter((command) => command.name === "Invoke-GitIdentity");
  assert.equal(closedCalls.length, 15, "exact reachable Invoke-ClosedProcess call count");
  assert.equal(gitCalls.length, 9, "exact reachable Invoke-GitIdentity call count");
  ...
  const closedDetails = closedCalls.map((command) => {
    ...
    for (const name of parameters.keys()) {
      assert.equal(closedAllowed.has(name), true, `Invoke-ClosedProcess parameter abbreviation: -${name}`);
    }
    for (const required of ["FilePath", "Arguments", "WorkingDirectory", "FailureCode"]) {
      assert.notEqual(parameters.get(required), null, `Invoke-ClosedProcess requires -${required}`);
      assert.equal(parameters.has(required), true, `Invoke-ClosedProcess requires -${required}`);
    }
    return { command, parameters, arguments: parameters.get("Arguments") };
  });
  const gitDetails = gitCalls.map((command) => {
    const parameters = commandParameters(command);
    assert.deepEqual([...parameters.keys()], ["Arguments", "FailureCode"], "Invoke-GitIdentity exact parameters");
    assert.notEqual(parameters.get("Arguments"), null, "Invoke-GitIdentity requires arguments");
    assert.notEqual(parameters.get("FailureCode"), null, "Invoke-GitIdentity requires failure code");
    return { command, parameters, arguments: parameters.get("Arguments") };
  });
  ...
  assert.equal(
    closedDetails.every((detail) =>
      ["ArrayExpressionAst", "VariableExpressionAst"].includes(detail.arguments.type)
    ),
    true,
    "closed process arguments must be an AST array or an admitted dataflow variable",
  );
  assert.equal(
    gitDetails.every((detail) => detail.arguments.type === "ArrayExpressionAst"),
    true,
    "Git identity arguments must be exact AST arrays",
  );

  const memberFlows = closedDetails.flatMap((detail) =>
    detail.arguments.members.map((member) => ({ owner: detail.command.owner, member }))
  );
  assert.deepEqual(
    memberFlows,
  ...
  assert.deepEqual(
    [...gitWrapper[0].parameters.entries()].map(([name, value]) => [name, value?.text ?? null]),
    [
  ...
  const gitDefinition = ast.functions.filter((definition) => definition.name === "Invoke-GitIdentity");
  ...
  assert.deepEqual(
    reachableProtectedAssignments.map(({ assignment, leftProtected, rightProtected }) => ({
  ...
  assert.deepEqual(
    reachableProtectedMemberInvocations.map(({ invocation, protectedVariables }) => ({
    ...
    [
      {
        ...
        memberText: "IndexOf",
        arguments: ["$record.sourceLiteral", "[System.StringComparison]::Ordinal"],
  ...
  assert.equal(queryLoopCalls.length, 1, "one closed process call inside exact queries loop");
  ...
  const imageRecords = imageTables.map((table) =>
    Object.fromEntries(table.entries.map((entry) => {
      assert.match(entry.value, /^'[^']*'$/u, `image ${entry.key} must be a literal`);
      return [entry.key, entry.value.slice(1, -1)];
    }))
  );
  assert.deepEqual(imageRecords, exactSupabaseImageRecords, "exact four immutable image records");
  assert.deepEqual(
    ast.returns.filter((item) => item.owner === "Get-ExactSupabaseImageRecords").map((item) => item.text),
    ["$expectedImages"],
  ...
  assert.deepEqual(
    cacheAssignments.map((assignment) => normalizePowerShellAstText(assignment.rightText)),
  ...
  const recordLoops = ast.foreachStatements.filter(
    (loop) => loop.owner === "Assert-LocalImageCache" && loop.variable === "record",
  );
  assert.deepEqual(recordLoops.map((loop) => loop.condition), ["$expectedImages"]);
  ...
  assert.equal(recordLoopCalls.length, 1, "one image inspect call inside exact record loop");
  assert.equal(
  ...
  const directArchive = closedDetails.filter((detail) => firstLiteral(detail) === "archive");
  const wrapperArchive = gitDetails.filter((detail) => firstLiteral(detail) === "archive");
  assert.equal(directArchive.length + wrapperArchive.length, 1, "one reachable archive producer");
  assert.equal(directArchive.length, 1, "archive producer is the direct closed process call");
  assert.equal(wrapperArchive.length, 0, "Git wrapper never produces an archive");
  assert.deepEqual(
    [...directArchive[0].parameters.entries()].map(([name, value]) => [name, value?.text ?? null]),
    [
      ["FilePath", "$gitExecutable"],
      ["Arguments", "@('archive', 'HEAD', '--format=tar', \"--output=$archivePath\", '--', 'supabase')"],
      ["WorkingDirectory", "$worktreeRoot"],
      ["FailureCode", "'A17_S1AR_GIT_ARCHIVE_FAILED'"],
    ],
    "one exact direct Supabase-only archive producer",
  );
  const tarExtract = closedDetails.filter((detail) => firstLiteral(detail) === "-xf");
  assert.equal(tarExtract.length, 1, "one reachable tar extractor");
  assert.deepEqual(
    [...tarExtract[0].parameters.entries()].map(([name, value]) => [name, value?.text ?? null]),
    [
...
function assertExactSupabaseArchiveBlock(ps) {
  ...
  assert.equal(typeof ps, "string", "archive validator requires full PowerShell source");
  assert.equal(
...
test("candidate changes are exact-four and accepted exact-five bytes remain immutable", () => {
  ...
  assert.deepEqual(changed, [...exactFour].sort());

  for (const [path, expected] of Object.entries(protectedExactFive)) {
...
test("PowerShell orchestrator binds immutable tools, archive child, ports and exact owned resources", () => {
  ...
  assert.match(ps, /supabase[\s\S]*stop[\s\S]*--project-id[\s\S]*a17-s1ar-20260827/iu);
...
test("orchestrator uses exact confirmation, seven production values and closed child permissions", () => {
  ...
  assert.match(ps, /--allow-net=127\.0\.0\.1:54321,127\.0\.0\.1:54329,127\.0\.0\.1:58017/u);
...
test("SQL harness proves live catalog ownership, forced RLS, zero policies and closed ACLs", () => {
  ...
  ) assert.equal(sql.toLowerCase().includes(marker.toLowerCase()), true, marker);
...
test("Deno harness is confirmation-ignored at Stage 1 and operationally binds BOOT/GUARD/ROT/LOCK/FETCH", () => {
  ...
  assert.match(deno, /Deno\.serve\(\{[\s\S]*hostname:\s*"127\.0\.0\.1"[\s\S]*port:\s*58017/iu);
...
test("the exact-three source is fail-closed against remote, broad teardown and secret projection", () => {
  const combined = [source(urls.powershell), source(urls.sql), source(urls.deno)]
    .join("\n");
  assert.doesNotMatch(
    combined,
  ...
  assert.doesNotMatch(combined, /LAIBE_ALLOWED_ORIGINS\s*=|Access-Control-Allow-Origin['"]?\s*:\s*['"]?\*/iu);
  assert.doesNotMatch(combined, /Write-(?:Host|Output)[^\n]*(?:SERVICE_ROLE|COOKIE_KEY|PROOF_KEY|SUPABASE_URL)/iu);
  assert.doesNotMatch(
    combined,
    /(?:^|[\\/'"`=])\.env(?:[.\s'"`]|$)|--env-file|credential\s+dump|docker\s+inspect/imu,
  );
  assert.doesNotMatch(combined, /localStorage|sessionStorage|document\.|window\.|location\./u);
...
test("R1 runtime identity is activation-supplied, revalidated, and followed by owned-runtime proof", () => {
  ...
  ]) assert.equal(ps.includes(dependency), true, `protected dependency ${dependency}`);
  assert.match(ps, /Assert-OwnedRuntimeState\s+-RequireExact/u);
  assert.match(ps, /docker[\s\S]*container[\s\S]*network[\s\S]*volume/iu);
  assert.match(ps, /Get-NetTCPConnection[\s\S]*OwningProcess[\s\S]*docker[\s\S]*port/iu);
  assertOrdered(
    ps,
    [
...
test("R2 cleanup is primary-first, exhaustive, transactional, and emits one terminal", () => {
  ...
  assert.match(ps, /if\s*\(\s*-not\s+\$preserveRuntimeRoot\s*\)[\s\S]*Remove-Item/u);
...
test("R3 SQL and Deno execute the admitted adversarial matrix instead of naming markers", () => {
  ...
  assert.match(sql, /expected_constraint_definitions\s*\([^)]*constraint_name[^)]*definition/iu);
  assert.match(sql, /expected_partial_indexes\s*\([^)]*index_name[^)]*definition/iu);
  assert.match(sql, /select\s+count\(\*\)[\s\S]*from expected_partial_indexes[\s\S]*is distinct from\s*3/iu);
  assert.match(sql, /coalesce\([^)]*relforcerowsecurity[^)]*,\s*false\)\s+is distinct from true/iu);
  assert.doesNotMatch(sql, /(?:projection|actual|value)\s*(?:<>|!=|=)\s*(?:expected|:'a17_)/iu);
  ...
  for (const authorityCase of ["invalid-status", "invalid-subject", "invalid-time"]) {
    assert.equal(sql.includes(authorityCase), true, authorityCase);
  }
  ...
  assert.match(sql, /before_function_definitions[\s\S]*is distinct from[\s\S]*after_function_definitions/iu);
  ...
  for (const tamperCase of ["cookie-bit-flip", "proof-bit-flip", "wrong-cookie-key", "wrong-proof-key", "malformed-cookie", "malformed-proof", "alg-none"]) {
    assert.match(deno, new RegExp(`await\\s+assertAcceptedRuntimeTamperCase\\(\\s*"${tamperCase}"`, "u"));
  }
  assert.match(deno, /bootstrapDependencies\.accessSessionVerifier\s*\.verifyAccessSession\(/u);
  ...
  assert.match(deno, /VERIFY_BARRIER_ROW_LOCKED[\s\S]*assertPendingBeforeRelease[\s\S]*VERIFY_BARRIER_RELEASED/iu);
  assert.match(deno, /assertSanitizedBodyless401[\s\S]*response\.status[\s\S]*responseBody\.byteLength/u);
});
...
test("R4 confirmation is consumed before any accepted production module is imported", () => {
  ...
  assert.match(deno, /else\s*\{[\s\S]*ignore:\s*true/u);
  assert.match(deno, /const productionEnvironment\s*=\s*exactSevenEnvironment\(/u);
  assert.deepEqual(
    [...new Set([...deno.matchAll(/productionEnvironment\.([A-Z][A-Z0-9_]+)/gu)].map((m) => m[1]))].sort(),
    [
      "LAIBE_DRS_APP_ORIGIN",
      "LAIBE_DRS_BFF_PROOF_KEY_V1",
      "LAIBE_DRS_SESSION_COOKIE_KEY_V1",
...
test("S2-R1 protected identity is exact-twelve and listener queries fail closed", () => {
  ...
  const protectedPaths = [...protectedFunction.matchAll(/'(supabase\/[^']+|tests\/[^']+)'/gu)]
    .map((match) => match[1])
    .sort();
  ...
  function assertListenerQueryFailsClosed(candidateSource) {
    const listenerStart = candidateSource.indexOf("function Assert-NoOwnedListeners");
    const listenerEnd = candidateSource.indexOf("function Get-ExactSupabaseImageRecords", listenerStart);
    ...
    assert.equal(listenerEnd > listenerStart, true, "listener slice must be forward and nonempty");
    ...
    assert.match(listenerFunction, /catch\s*\{[\s\S]*A17_S1AR_LISTENER_QUERY_FAILED/u);
  }
  ...
  assert.notEqual(mutatedListenerSource, ps, "mutation must remove the listener-local flag");
  ...
  assert.match(laterListenerSource, /-ErrorAction\s+Stop/u, "mutation fixture must retain later occurrences");
  assert.throws(
...
test("S2-R2 causal terminal and cleanup preserve an exact ROT or LOCK reason", () => {
  ...
  assert.match(ps, /function Read-ExactCausalVerdict[\s\S]*Matches\([\s\S]*Count\s+-ne\s+1/u);
  ...
  assert.match(deno, /delete from integration\.drs_workspace_grants[\s\S]*casework_case_id\s+in/iu);
  assert.doesNotMatch(deno, /delete from integration\.drs_workspace_grants[\s\S]{0,300}\band case_id\s+in/iu);
...
test("S2-R3 issue verify revoke lock lanes and exact column metadata are executable", () => {
  const sql = source(urls.sql);
  const deno = source(urls.deno);
  assert.match(sql, /expected_column_metadata\s*\([\s\S]*ordinal[\s\S]*column_name[\s\S]*format_type[\s\S]*attnotnull[\s\S]*default_expression[\s\S]*identity_kind[\s\S]*generated_kind/iu);
  assert.match(sql, /expected_column_metadata[\s\S]*except[\s\S]*pg_attribute[\s\S]*except[\s\S]*expected_column_metadata/iu);
  assert.match(sql, /coalesce\([^)]*attnotnull[^)]*,\s*false\)/iu);

  const issueLane = deno.slice(deno.indexOf("async function assertIssueLockBarrier"), deno.indexOf("async function assertVerifyLockBarrier"));
  ...
  const verifyLane = deno.slice(deno.indexOf("async function assertVerifyLockBarrier"), deno.indexOf("async function assertRevokeLockBarrier"));
  ...
  const revokeLane = deno.slice(revokeStart, deno.indexOf("if (runtimeConfirmed)", revokeStart));
  assert.match(revokeLane, /integration\.drs_server_sessions[\s\S]*for update/iu);
  ...
  for (const call of ["assertIssueLockBarrier(", "assertVerifyLockBarrier(", "assertRevokeLockBarrier("]) {
    assert.equal(occurrences(deno, new RegExp(call.replace("(", "\\("), "gu")) >= 2, true, call);
  }
});
...
test("S2-R4 hostile fetch and tamper run through accepted runtime ports with output canary scans", () => {
  ...
  assert.match(fixture, /createDrsSecureSessionRuntime\([\s\S]*fetch:\s*globalThis\.fetch/u);
  assert.match(fixture, /hostileRuntime\.bootstrapDependencies\.accessSessionVerifier[\s\S]*verifyAccessSession/u);
  ...
  ]) assert.match(deno, new RegExp(`await\\s+assertAcceptedRuntimeHostileCase\\(\\s*"${hostileCase}"`, "u"));
  ...
  assert.match(ps, /function Assert-CapturedOutputSanitized[\s\S]*SUPABASE_SERVICE_ROLE_KEY[\s\S]*LAIBE_DRS_SESSION_COOKIE_KEY_V1[\s\S]*LAIBE_DRS_BFF_PROOF_KEY_V1/u);
  assert.match(ps, /fixtureIds[\s\S]*authorityCanaries[\s\S]*A17_S1AR_CAPTURED_OUTPUT_SECRET_OR_CANARY/u);
  assert.match(ps, /Assert-CapturedOutputSanitized[\s\S]*\$denoResult\.Stdout[\s\S]*\$denoResult\.Stderr/u);
});
...
test("S3-F1 hostile fetch uses a valid digest input and proves one expected RPC path per case", () => {
  ...
  assert.equal(ps.includes(hostileToken), true, "fixed canary is output-scanned");
});
...
test("S3-F2 causal ROT or LOCK remains first when cleanup also fails", () => {
  ...
  assertOrdered(
    terminal,
    ["primaryError?.causalMarker", "primaryTrace", "...cleanupErrors", "new AggregateError"],
    "causal marker precedes cleanup failures",
  );
  const psTerminal = ps.slice(ps.indexOf("$trace ="), ps.indexOf("exit $terminalExitCode"));
  assert.match(
    psTerminal,
...
test("S3-F3 every lock lane proves a future DB deadline and releases only after DB expiry", () => {
  ...
  for (const [laneName, nextLane, prefix, deadlineColumn, operation] of [
    ["assertIssueLockBarrier", "assertVerifyLockBarrier", "ISSUE", "valid_until =", "createVerifiedSession("],
    ["assertVerifyLockBarrier", "assertRevokeLockBarrier", "VERIFY", "valid_until =", ".verifyAccessSession("],
    ["assertRevokeLockBarrier", "if (runtimeConfirmed)", "REVOKE", "expires_at =", "sessionRevoker.revokeServerSession("],
  ]) {
...
test("S3-F4 cleanup readback proves every task-owned database fixture is absent", () => {
  ...
  for (const relation of [
    ...
    assert.equal(
      occurrences(cleanup, new RegExp(relation.replaceAll(".", "\\."), "giu")) >= 2,
      true,
...
test("S3-F5 cookie revoke and proof-window lifecycle is exact and executable", () => {
  ...
  assert.match(createCookie, /response\.headers\.get\("location"\)[\s\S]*successUrl/u);
  assert.match(createCookie, /response\.arrayBuffer\(\)[\s\S]*byteLength[\s\S]*0/u);
  ...
  assert.match(revoked, /assert\.rejects\([\s\S]*sessionRevoker\.revokeServerSession/u);
...
test("S3-F6 RLS closure denies every table privilege to all admitted API roles", () => {
  ...
  assert.match(sql, /relrowsecurity[\s\S]*relforcerowsecurity[\s\S]*pg_policies/iu);
});
...
test("S6-F2 stdin write flush and exit consume one total finite process deadline", () => {
  ...
  assert.equal(
    occurrences(closedProcess, /Get-RemainingProcessDeadlineMilliseconds\s+-ProcessDeadline \$processDeadline\s+-ProcessTimeoutMilliseconds \$ProcessTimeoutMilliseconds/gu),
    3,
...
test("S9-F1 image inspect stdout uses absolute anchors before trim and split", () => {
  ...
  assert.equal(guards.length, 1, "inspect stdout must have one closed shape guard");
  assert.equal(
    guards[0][1],
    String.raw`\A[^\r\n]+(?:\r?\n)?\z`,
...
test("S10-F1 listener selector is bounded and mutation-sensitive", () => {
  ...
  assertOrdered(
    ...
    [
      ...
      "assert.equal(listenerEnd > listenerStart, true",
      "const listenerFunction = candidateSource.slice(listenerStart, listenerEnd)",
      'const mutatedListenerSource = ps.replace(',
      '"Get-NetTCPConnection -State Listen -ErrorAction Stop"',
      'const laterListenerSource = mutatedListenerSource.slice(',
      'mutatedListenerSource.indexOf("function Get-ExactSupabaseImageRecords")',
      "assert.match(laterListenerSource, /-ErrorAction\\s+Stop/u",
      "assert.throws(",
      "assertListenerQueryFailsClosed(mutatedListenerSource)",
    ],
...
test("S11-F1 Git Tar and Deno identities are immutable and gated before use", () => {
  ...
  const preUseStart = ps.indexOf("$scriptRoot = Split-Path -Parent $PSCommandPath");
...
test("S12-F1 every closed-process argument array is a legal named-parameter value", () => {
  ...
  for (const site of correctedSites) {
    assert.equal(
      occurrences(ps, new RegExp(site.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "gu")),
      1,
...
test("S13-F1 runtime archive is an exact immutable Supabase-only pathspec", () => {
  ...
  assert.equal(
    occurrences(ps, /\$ArchiveContract\s*-ne\s*'git archive HEAD -- supabase'/gu),
    1,
    "archive drift guard must bind the same exact contract",
  );

  const archiveStart = ps.indexOf("$archivePath = Assert-ExactDescendant");
  const archiveEnd = ps.indexOf("Remove-Item -LiteralPath $archivePath -Force", archiveStart);
...
test("S16-F1 native AST contract closes reachable invocation grammar and exact dataflows", () => {
  ...
  const mutations = [
    ...
    [
      ...
      appendAfterRemoval(ps, "$Arguments = @('archive','HEAD')", "Arguments variable rebinding"),
    ],
    [
      "Arguments index rebinding",
      appendAfterRemoval(ps, "$Arguments[0] = 'archive'", "Arguments index rebinding"),
    ],
    [
      "Arguments member mutation",
      appendAfterRemoval(ps, "$Arguments.SetValue('archive', 0)", "Arguments member mutation"),
    ],
    [
      "queries index rebinding",
      appendAfterRemoval(ps, "$queries[0] = @('images','ls')", "queries index rebinding"),
    ],
    [
