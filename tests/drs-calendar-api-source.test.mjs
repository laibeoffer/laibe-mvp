  ...
  for (const path of endpointPaths) {
    ...
    if (route) {
      ...
      for (const backendMarker of [
        ...
        if (backend !== -1) assert.ok(guard < backend, `${path}: ${backendMarker}`);
      }
    } else {
...
test("owner and vendor endpoint wrappers stay excluded from the DRS composition", async () => {
  ...
  const admittedDrsSource = (
    await Promise.all([
      source("supabase/functions/_shared/google-calendar/drs-specialist-authorization.ts"),
...
test("verify_jwt false is paired with the closed BFF proof and current-authority guard", async () => {
  ...
  const proof = bootstrap.indexOf("const proof = bearerProof(request)", closedRequest);
  ...
  const authority = bootstrap.indexOf("authorizationFactsDigest(session)", session);
  assert.ok(
