...
import { createDrsGoogleSessionRuntime } from "../functions/_shared/drs-auth/drs-google-session-runtime.ts";
...
test("focused RED: production Google runtime is absent", async () => {
  const runtimeUrl = new URL(
    "../functions/_shared/drs-auth/drs-google-session-runtime.ts",
    import.meta.url,
  );
  const runtimeSource = await Deno.readTextFile(runtimeUrl);
  const startSource = await Deno.readTextFile(
    new URL("../functions/drs-google-auth-start/index.ts", import.meta.url),
  );
  const callbackSource = await Deno.readTextFile(
    new URL("../functions/drs-google-auth-callback/index.ts", import.meta.url),
  );
  const grantSource = await Deno.readTextFile(
    new URL("../functions/drs-session-grant/index.ts", import.meta.url),
  );

  assert.match(runtimeSource, /createDrsGoogleSessionRuntime/u);
  assert.match(runtimeSource, /createGoogleIdentityAdapter/u);
  assert.match(runtimeSource, /createDrsSecureSessionRuntime/u);
  for (const sourceText of [startSource, callbackSource, grantSource]) {
    assert.match(sourceText, /drs-google-session-runtime\.ts/u);
    assert.match(sourceText, /createDrsGoogleSessionRuntime/u);
  }
});

function productionRuntimeEnvironment(overrides = {}) {
  const key = (byte) => {
    const binary = String.fromCharCode(...new Uint8Array(32).fill(byte));
  ...
  const values = {
    ...
    SUPABASE_SERVICE_ROLE_KEY:
      "service-role-key-that-is-longer-than-thirty-two-bytes",
    LAIBE_DRS_APP_ORIGIN: ORIGIN,
    LAIBE_DRS_SESSION_SUCCESS_URL: `${ORIGIN}/pcm/console`,
    LAIBE_DRS_SESSION_COOKIE_NAME: "__Host-laibe_drs_session",
    LAIBE_DRS_SESSION_COOKIE_KEY_V1: key(1),
    LAIBE_DRS_BFF_PROOF_KEY_V1: key(2),
    LAIBE_DRS_GOOGLE_CLIENT_ID: "google-client-id.apps.exampleusercontent.com",
    LAIBE_DRS_GOOGLE_CLIENT_SECRET: "google-client-secret-value",
    LAIBE_DRS_GOOGLE_REDIRECT_URL:
      `${ORIGIN}/functions/v1/drs-google-auth-callback`,
    LAIBE_DRS_IDENTITY_STATE_KEY_V1: key(3),
    ...overrides,
  };
  return { get: (name) => values[name] };
}

test("production Google runtime fails closed on missing environment and creates digest-only state through the exact public wrapper", async () => {
  assert.equal(
    createDrsGoogleSessionRuntime({
      env: productionRuntimeEnvironment({ LAIBE_DRS_GOOGLE_CLIENT_SECRET: "" }),
    }).runtimeAvailable,
    false,
  );
  ...
  const runtime = createDrsGoogleSessionRuntime({
    env: productionRuntimeEnvironment(),
    fetch,
    now: () => NOW,
  });
  assert.equal(runtime.runtimeAvailable, true);
  const response = await runtime.googleAuthStartDependencies.adapter.start(
    new Request(`${ORIGIN}/functions/v1/drs-google-auth-start`, {
      method: "POST",
      headers: { origin: ORIGIN, "content-type": "application/json" },
      body: "{}",
    }),
  ...
  assert.equal(
    calls[0].url,
    "https://project.supabase.co/rest/v1/rpc/drs_identity_link_state_create_v1",
  );
  const stored = JSON.parse(calls[0].init.body);
  assert.match(stored.p_state_digest, /^[A-Za-z0-9_-]{43}$/u);
  assert.match(stored.p_nonce_digest, /^[A-Za-z0-9_-]{43}$/u);
  assert.match(stored.p_pkce_verifier_ciphertext, /^v1\./u);
  assert.equal(
    calls[0].init.body.includes(authorization.searchParams.get("state")),
    false,
  );
  assert.equal(
    calls[0].init.body.includes(authorization.searchParams.get("nonce")),
    false,
  );
  assert.equal(stored.p_intended_action, "login");
  assert.equal(stored.p_authenticated_user_id, null);
  assert.equal(stored.p_specialist_id, null);
  assert.equal(stored.p_authorization_subject, null);
  assert.ok(
    Date.parse(stored.p_expires_at) - Date.parse(stored.p_now) <=
      15 * 60 * 1000,
  );
  assert.equal(JSON.stringify(result).includes("service-role-key"), false);
  assert.equal(JSON.stringify(result).includes("google-client-secret"), false);
});

test("production session-grant runtime keeps gateway JWT and server-resolved authority separate from browser case facts", async () => {
  const calls = [];
  const fetch = (url, init = {}) => {
    calls.push({ url: String(url), init });
    if (String(url).endsWith("/auth/v1/user")) {
      return Promise.resolve(Response.json({ id: AUTH_USER_A }));
    }
    if (String(url).endsWith("/rest/v1/rpc/drs_workspace_grant_v1")) {
      return Promise.resolve(Response.json(activeWorkspaceGrant()));
    }
  ...
  const handler = createDrsSessionGrantHandler(
    runtime.sessionGrantDependencies,
  );
  const accepted = await handler(
    new Request(`${ORIGIN}/functions/v1/drs-session-grant`, {
      method: "POST",
      headers: {
        origin: ORIGIN,
  ...
  assert.equal((await accepted.json()).case.id, CASE_A);
  ...
  assert.deepEqual(rpcBody, {
    p_authenticated_user_id: AUTH_USER_A,
    p_expected_case_id: null,
    p_authorization_subject: null,
  });
  const injected = await handler(
    new Request(`${ORIGIN}/functions/v1/drs-session-grant?caseId=${CASE_B}`, {
      method: "POST",
      headers: {
        origin: ORIGIN,
        authorization: "Bearer gateway-jwt",
