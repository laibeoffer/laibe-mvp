...
-- Immutable manifest admitted for the protected exact-seven remote baseline.
-- ACLs are normalized to role names and effective privileges so database-local
-- role OIDs and aclitem ordering cannot create false drift.
create temporary table drs_bridge_expected_fingerprint as
select
  'postgres'::text as v_expected_cases_owner,
  'postgres'::text as v_expected_members_owner,
  '28891843d5ac2d59ebc1e15479d8db81ec53ff8dfe738c2c6be2cbd4c202391e'::text
    as v_expected_cases_acl_fingerprint,
  '28891843d5ac2d59ebc1e15479d8db81ec53ff8dfe738c2c6be2cbd4c202391e'::text
    as v_expected_members_acl_fingerprint,
  true as v_expected_cases_rls,
  true as v_expected_members_rls,
  false as v_expected_cases_force_rls,
  false as v_expected_members_force_rls,
  'a19ea95bc85c2cc4e8c042198b3694e9a64d0d8fc3180eb5b9443ec432584072'::text
    as v_expected_cases_trigger_fingerprint,
  '4df20d823f8a8f961fa61b98cb6da4ad5032186b44d77d187af3e0a7baf4e088'::text
    as v_expected_members_trigger_fingerprint,
  '0238bc106b994deb4a0e5fd6cd9ef4485d3c4c1982ea5a8a38d916e17da6df4d'::text
    as v_expected_cases_constraint_fingerprint,
  '6324a7ce0d715801be82aa9ad646eab392feb991a233b5cf8c7e9e1111907afd'::text
    as v_expected_members_constraint_fingerprint,
  'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855'::text
    as v_expected_cases_policy_fingerprint,
  'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855'::text
    as v_expected_members_policy_fingerprint,
  'abf0355c5a6ccabf19ca176ab4843b9b3461a0ccdfb0b1b4ca5ec0fe7a2ce10e'::text
    as v_expected_calendar_definition_fingerprint,
  'postgres'::text as v_expected_calendar_owner,
  '2bb79e66093b29431b55e1200472283b8a92af48e435c12637cc242f5a4b5588'::text
    as v_expected_calendar_acl_fingerprint;

create temporary view drs_bridge_current_fingerprint as
with protected_relations as (
  select
    ...
    relation_record.relforcerowsecurity as force_rls,
    (
      select pg_catalog.encode(extensions.digest(pg_catalog.convert_to(
        coalesce(pg_catalog.string_agg(acl_item, E'\n' order by acl_item), ''),
        'UTF8'
      ), 'sha256'), 'hex')
      from (
    ...
    (
      select pg_catalog.encode(extensions.digest(pg_catalog.convert_to(
        coalesce(pg_catalog.string_agg(trigger_item, E'\n' order by trigger_item), ''),
        'UTF8'
      ), 'sha256'), 'hex')
      from (
        select pg_catalog.jsonb_build_array(
          coalesce(constraint_record.conname, trigger_record.tgname),
    ...
    (
      select pg_catalog.encode(extensions.digest(pg_catalog.convert_to(
        coalesce(pg_catalog.string_agg(constraint_item, E'\n' order by constraint_item), ''),
        'UTF8'
      ), 'sha256'), 'hex')
      from (
        select pg_catalog.jsonb_build_array(
        ...
        )::text as constraint_item
        from pg_catalog.pg_constraint constraint_record
        where constraint_record.conrelid = relation_record.oid
      ) normalized_constraints
    ) as constraint_fingerprint,
    (
      select pg_catalog.encode(extensions.digest(pg_catalog.convert_to(
        coalesce(pg_catalog.string_agg(policy_item, E'\n' order by policy_item), ''),
        'UTF8'
      ), 'sha256'), 'hex')
      from (
        select pg_catalog.jsonb_build_array(
          ...
          (
            ...
            from (
              ...
              from pg_catalog.unnest(policy_record.polroles) role_oid
            ) normalized_policy_roles
          ),
        ...
        where policy_record.polrelid = relation_record.oid
      ) normalized_policies
  ...
  where relation_namespace.nspname = 'casework'
    and relation_record.relname in ('cases', 'case_members')
), calendar_function as (
  select
    pg_catalog.pg_get_userbyid(function_record.proowner) as owner_name,
    pg_catalog.encode(extensions.digest(pg_catalog.convert_to(
      pg_catalog.pg_get_functiondef(function_record.oid), 'UTF8'
    ), 'sha256'), 'hex') as definition_fingerprint,
    (
      select pg_catalog.encode(extensions.digest(pg_catalog.convert_to(
        coalesce(pg_catalog.string_agg(acl_item, E'\n' order by acl_item), ''),
        'UTF8'
  ...
  where function_record.oid =
    'integration.google_calendar_drs_authorize_transaction_v1(uuid,uuid,text,text)'::pg_catalog.regprocedure
)
select
  (select owner_name from protected_relations where relname = 'cases') as cases_owner,
  (select owner_name from protected_relations where relname = 'case_members') as members_owner,
  (select acl_fingerprint from protected_relations where relname = 'cases') as cases_acl_fingerprint,
  (select acl_fingerprint from protected_relations where relname = 'case_members') as members_acl_fingerprint,
  (select rls from protected_relations where relname = 'cases') as cases_rls,
  (select rls from protected_relations where relname = 'case_members') as members_rls,
  (select force_rls from protected_relations where relname = 'cases') as cases_force_rls,
  (select force_rls from protected_relations where relname = 'case_members') as members_force_rls,
  (select trigger_fingerprint from protected_relations where relname = 'cases') as cases_trigger_fingerprint,
  (select trigger_fingerprint from protected_relations where relname = 'case_members') as members_trigger_fingerprint,
  (select constraint_fingerprint from protected_relations where relname = 'cases') as cases_constraint_fingerprint,
  (select constraint_fingerprint from protected_relations where relname = 'case_members') as members_constraint_fingerprint,
  (select policy_fingerprint from protected_relations where relname = 'cases') as cases_policy_fingerprint,
  (select policy_fingerprint from protected_relations where relname = 'case_members') as members_policy_fingerprint,
  (select definition_fingerprint from calendar_function) as calendar_definition_fingerprint,
  (select owner_name from calendar_function) as calendar_owner,
  (select acl_fingerprint from calendar_function) as calendar_acl_fingerprint;
...
declare
  ...
  v_actual record;
begin
  select * into strict v_expected from drs_bridge_expected_fingerprint;
  select * into strict v_actual from drs_bridge_current_fingerprint;

  if v_actual.cases_owner is distinct from v_expected.v_expected_cases_owner
    or v_actual.members_owner is distinct from v_expected.v_expected_members_owner
    or v_actual.cases_acl_fingerprint is distinct from v_expected.v_expected_cases_acl_fingerprint
    or v_actual.members_acl_fingerprint is distinct from v_expected.v_expected_members_acl_fingerprint
    or v_actual.cases_rls is distinct from v_expected.v_expected_cases_rls
    or v_actual.members_rls is distinct from v_expected.v_expected_members_rls
    or v_actual.cases_force_rls is distinct from v_expected.v_expected_cases_force_rls
    or v_actual.members_force_rls is distinct from v_expected.v_expected_members_force_rls
    or v_actual.cases_trigger_fingerprint is distinct from v_expected.v_expected_cases_trigger_fingerprint
    or v_actual.members_trigger_fingerprint is distinct from v_expected.v_expected_members_trigger_fingerprint
    or v_actual.cases_constraint_fingerprint is distinct from v_expected.v_expected_cases_constraint_fingerprint
    or v_actual.members_constraint_fingerprint is distinct from v_expected.v_expected_members_constraint_fingerprint
    or v_actual.cases_policy_fingerprint is distinct from v_expected.v_expected_cases_policy_fingerprint
    or v_actual.members_policy_fingerprint is distinct from v_expected.v_expected_members_policy_fingerprint
    or v_actual.calendar_definition_fingerprint is distinct from v_expected.v_expected_calendar_definition_fingerprint
    or v_actual.calendar_owner is distinct from v_expected.v_expected_calendar_owner
    or v_actual.calendar_acl_fingerprint is distinct from v_expected.v_expected_calendar_acl_fingerprint
  then
    raise exception 'DRS_REMOTE_BASELINE_SCHEMA_MANIFEST_MISMATCH';
  end if;
end;
...
drop view drs_bridge_current_fingerprint;
drop table drs_bridge_expected_fingerprint;
