...
type NotificationClaim = Readonly<{
  ...
  admitted: false;
  state: "empty" | "suppressed_authority" | "temporarily_unavailable" | "permission_denied";
}>;
...
function safeHttps(value: unknown): value is string {
  if (!safeText(value, 1, 512)) return false;
  try {
...
function readClaim(input: unknown): NotificationClaim {
  if (exactOwnKeys(input, ["admitted", "state"]) && own(input, "admitted") === false) {
    ...
    if (["empty", "suppressed_authority", "temporarily_unavailable", "permission_denied"].includes(String(state))) {
  ...
  const caseUrl = own(input, "caseUrl");
  if (
    ...
    !safeText(nextAction, 1, 160) || !safeHttps(caseUrl)
  ) throw new Error("invalid_notification_claim");
