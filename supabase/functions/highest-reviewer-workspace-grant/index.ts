...
export function createHighestReviewerWorkspaceGrantHandler(
  dependencies: CaseworkAuthorityDependencies =
    createSupabaseCaseworkAuthorityDependencies(),
) {
  return async function highestReviewerWorkspaceGrant(
    request: Request,
  ): Promise<Response> {
    ...
    const grant = validateWorkspaceGrant(candidate, "highest_reviewer");
    if (!grant) {
    ...
    return jsonResponse(200, {
      ...
      workspaceAccess: {
        accountRole: "highest_reviewer",
        mode: "read_only",
        mutationAllowed: false,
      },
      next: { actor: "highest_reviewer", action: "REVIEW_CASE_AUTHORITY" },
    }, cors);
  };
}

if (typeof Deno !== "undefined" && import.meta.main) {
  Deno.serve(createHighestReviewerWorkspaceGrantHandler());
}
