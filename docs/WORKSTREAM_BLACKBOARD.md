...
## Agent Self-Introduction: Budget Review Gate Agent
...
Role:
建立候選資料、價格區間、HandoffProposal、MethodSpec proposal、Output feedback proposal 的審核閘門與決策紀錄。確保 candidate / proposal 只能進 review queue，不能直接變正式價格或正式規則。

Primary Outputs:
- review gate contract
- review queue schema
- review decision contract
- approval / rejection / needs-info rules
- reviewer decision log
- candidate-to-formal forbidden flow list
...
Need Commander:
No，除非要決定正式價格核准權、正式報價權或人工審核政策。

Need Reviewer:
Yes if any proposal attempts to become formal price, PricingRule, MaterialSpec, LaborRule, or BudgetEstimateLine.

GitHub Source Note:
LOCAL_STATE_IGNORED_GITHUB_IS_SOURCE_OF_TRUTH. The current initialization uses GitHub `main` at `4cb9fe9d902fbd6c4eed16c525629e03ab0c57a1` as source of truth.
...
- Workstream: budget/review-gate
- Branch / Repo: `budget/review-gate` / `laibeoffer/laibe-mvp`
- Status: `ACTIVE_INITIALIZATION / DOCS_ONLY_REVIEW_GATE_CONTRACT`
- Changed: Added the Budget Review Gate self-introduction and initialized docs-only review gate contracts, review queue schema, review decision contract, candidate-to-formal approval policy, reviewer decision log, forbidden direct publish flows, and sample review artifacts.
- Files: `docs/WORKSTREAM_BLACKBOARD.md`, `docs/budget_review_gate/`
- PR / Commit: pending PR on `budget/review-gate`
- Blocked: None.
- Next: Open PR for Deputy / integration visibility. Do not produce formal prices, PricingRule, MaterialSpec, LaborRule, BudgetEstimateLine, BudgetOutputSnapshot, customer quote, payment, AI API, DB / Supabase, n8n, or implementation changes.
- Need Commander: No.
- Need Reviewer: Yes if any proposal attempts to become formal price, PricingRule, MaterialSpec, LaborRule, or BudgetEstimateLine.
