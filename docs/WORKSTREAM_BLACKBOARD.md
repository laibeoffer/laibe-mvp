...
## Agent Self-Introduction: LAIBE_VISUAL_SIM

Agent:
模擬圖生成聊天室 / LAIBE_VISUAL_SIM

Workstream:
visual/simulation-governance

Repo / Branch:
laibeoffer/laibe-mvp / visual/simulation-governance

Completion Claim:
100%

Role:
負責萊比網站中的風格示意圖 / 模擬圖治理系統，協助甲方在案件上架前，將模糊的風格描述轉換為 visual brief、prompt、negative prompt、sandbox preview 與非正式示意圖任務狀態。

Scope:
- `docs/ai_style_visual_chat/`
- `templates/LAIBE_VISUAL_SIM_TASK_TEMPLATE.md`
- `skills/laibe-visual-sim/SKILL.md`
- 若已存在，僅可讀取 `preview_floor_plan` 中與 visual simulation panel 有關的 mock / sandbox 接口
...
- reviewer checklist
- disclaimer / non-production usage rule

Web Runtime Requirement:
此系統必須能在萊比網站流程中順利被展示或被呼叫，至少要能在對應頁面中呈現：
- AI 風格示意入口或 panel
- prompt preview 或 visual brief preview
- sandbox / mock preview 狀態
- 固定 disclaimer
- reference image disabled notice
- 不接真 image API 的安全狀態
- 不污染 Plancraft geometry
- 不寫入正式案件資料
- 不進 budget official data
...
- 不接真 AI API
- 不接真上傳
- 不寫入 production assets
- 不寫入正式案件資料
- 不污染 Plancraft geometry
- 不修改 `project.walls` / `project.openings` / `project.zones` / `project.scale` / `plancraftBridge`
- 不進 `BudgetOutputSnapshot`
- 不進 Renderer
...
Completion Evidence:
- PR #6 `Add visual simulation governance docs`: merged.
- PR #24 `Add visual prompt sandbox governance packet`: merged; merge commit `cf170e248a48be2df43f6cd6e6db0ef956cd5658`; Issue #19 closed by merge.
- Governance files present under `docs/ai_style_visual_chat/`: `GENERATED_IMAGE_STORAGE_POLICY.md`, `IMAGE_API_REQUEST_SCHEMA.md`, `IMAGE_API_SPIKE_GOVERNANCE.md`, `IMAGE_API_SPIKE_REVIEW_CHECKLIST.md`, `PROMPT_SANITIZATION_RULES.md`, `REFERENCE_IMAGE_POLICY.md`, `VISUAL_BRIEF_PROMPT_SANDBOX_PACKET.md`.
- Template evidence: `templates/LAIBE_VISUAL_SIM_TASK_TEMPLATE.md`.
- Skill evidence: `skills/laibe-visual-sim/SKILL.md`.
- Web panel / mock UI / sandbox preview evidence: `src/stitch_laibe_landing_onboarding/preview_floor_plan/code.html` and `src/stitch_laibe_landing_onboarding/preview_floor_plan/plan-puzzle.js` are the recorded Visual Simulation Panel / disabled fallback / sandbox preview interface locations; this registration task did not modify them.
- Runtime smoke evidence: local project-root HTTP smoke loaded `http://127.0.0.1:8770/src/stitch_laibe_landing_onboarding/preview_floor_plan/code.html`; desktop and mobile checks rendered the mock / sandbox UI without detected `ERR_`, `Uncaught`, `ReferenceError`, `TypeError`, `SyntaxError`, or `Failed` browser-output strings.
- Runtime verification is mock / sandbox only: no real image API, backend proxy, API key, upload pipeline, production asset, formal case write, Plancraft geometry write, `BudgetOutputSnapshot`, or Renderer path is enabled.
