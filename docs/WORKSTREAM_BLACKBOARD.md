...
|---|---|---|---:|---|---|---|---|---|---|---|
| Plan Puzzle UI | `plancraft/page-ui` | `laibeoffer/laibe-mvp` / `codex/plan-puzzle-intuitive-workspace-repair-0-15-2` | 100 | BROWSER_VALIDATED | PR #67 base / 0.15.2 repair branch | `0.15.2-intuitive-workspace-repair` repairs compact workspace usability: 60px topbar, 56px icon rail, movable/collapsible floating palette, 4-tab inspector, tab hit-target fix, empty canvas CTA, focus mode canvas expansion; `node --check` passed; browser console error count 0 at `http://127.0.0.1:50367/code.html?validation=intuitive-workspace-repair-0-15-2` | None for scoped implementation | No for scoped implementation; Yes for PR landing | No by default | Commit, push, and open scoped draft PR |
