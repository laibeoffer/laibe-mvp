...
| Output Documents | Issue #18 / PR #23 | Output Documents Builder | `output/renderer-static-guard-review-packet` | `a75e380` | `https://github.com/laibeoffer/laibe-mvp/pull/23` | `SYNC_BLOCKED_AFTER_MAIN_ADVANCE` | PR #23 had clean Codex result `4531569296` on head `a75e380`, but latest `origin/main` has advanced through `24e0c72`; GitHub PR metadata now reports `mergeable=false` and base `0e8ab82`, while the available merge ref is stale against current main. | Prior checks are stale against latest main. Re-sync latest `origin/main`, rerun renderer static guard / syntax / mismatch / fixture checks, and request Codex re-review if the head changes. | 0 | Executive Officer + Output Documents Builder | Withdraw final gate until the branch is based on current main and GitHub reports mergeable / merge ref against latest main. | MEDIUM | No | No unless new Codex review reports NEEDS_FIX / P1 / P2 or scope drifts |
