...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Plan Puzzle | Issue #15 / PR #25 | Plan Puzzle Builder | `plancraft/zone-area-boundary-refinement` | `ffbe8e1` | `https://github.com/laibeoffer/laibe-mvp/pull/25` | `DEPUTY2_WORKFLOW_REPAIR_ASSIGNED / CURRENT_MAIN_HANDOFF_CONFLICT` | PR #25 received repeated local-only Codex connector handoff comments `4531872891` / `4531949297`; reported local commits `33c4695` / `d8e2c4e` are not pushed to PR #25. Current Deputy patrol on `origin/main` `a215335` confirms no `refs/pull/25/merge` exists and `git merge-tree --write-tree origin/main refs/remotes/origin/pr/25` exits `1` with `docs/NEXT_CODEX_HANDOFF.md` conflict. | Prior `node --check` / guard checks were reported in PR comment `4531558124`; the local-only handoffs also report `node --check` pass, but final delivery evidence requires a pushed current-main repair and `refs/pull/25/merge` before Codex review. | 4 | Deputy Codex-2 | Deputy Codex-2 repair assignment remains active: resolve the current-main handoff conflict inside Issue #15 scope, push an actual branch update, rerun `node --check src/stitch_laibe_landing_onboarding/preview_floor_plan/plan-puzzle.js` and guard checks, and request Codex review only after merge ref exists. Stop if scope drifts. | MEDIUM / DEPUTY_DECISION_MADE | No | No unless scope drift or Codex review reports NEEDS_FIX / P1 / P2 |
| Raw Candidate | Issue #17 / PR #26 | Raw Candidate Builder | `warehouse/raw-source-quality-scoring` | `7853fe7` | `https://github.com/laibeoffer/laibe-mvp/pull/26` | `VALIDATION_REFRESH_FOUND / CURRENT_MAIN_SIMULATION_PASS / DEPUTY_FINAL_GATE_CANDIDATE` | PR #26 previously had Executive validation `4531540239`, Codex clean result `4531555287`, and Deputy gate routing `4531573641`. The missing current-main validation refresh was posted in PR comment `4532187707` against latest `origin/main` `f960cfda01beca5d3d61d8065094bba8a95b48df`; local `git merge-tree --write-tree origin/main refs/remotes/origin/pr/26/head` exits `0` and produced tree `7650c6a3cd615004fa0244c0780312cb6104b935`. | Current-main R1.5 validation refresh reported pass: source quality scoring, raw warehouse static guard, syntax checks, prior raw demos, proposal contract, warehouse export safety, observed price safety, and forbidden formal-pricing checks. Evidence remains candidate-only: no formal price, no `PricingRule`, no formal `MaterialSpec` / `LaborRule`, no `BudgetEstimateLine.unit_price`, no renderer / Excel / PDF / `BudgetOutputSnapshot`, no floor-plan / frontend / Supabase/API/migration / RAG / AI API / payment / escrow / listing fee. | 0 | Deputy Codex | Stop ordinary Executive chase unless branch head changes or validation evidence is contradicted. Deputy owns final clean-scope merge / reject gate; request Codex re-review only if PR head changes or scope drifts. | MEDIUM / DEPUTY2_REFRESH_ACCEPTED | No | No unless new Codex review reports NEEDS_FIX / P1 / P2, formal-price risk, or scope drift |
| MethodSpec | Issue #16 / PR #22 | MethodSpec Builder | `warehouse/method-spec-validator-freeze-note` | `e338431` | `https://github.com/laibeoffer/laibe-mvp/pull/22` | `DEPUTY_SIGNAL_ACCEPTED / FINAL_GATE_CANDIDATE_CURRENT_MAIN_SIMULATION_PASS` | PR #22 has clean Codex result `4531356014`. Current Deputy patrol on `origin/main` `a215335` finds local `git merge-tree --write-tree origin/main refs/remotes/origin/pr/22` exits `0`, and base-to-head changed files remain limited to Issue #16 allowed docs: `docs/budget/32-method-spec-validator-freeze-note.md`, `docs/NEXT_CODEX_HANDOFF.md`, and `docs/CURRENT_PHASE_REVIEW_PACKET.md`. | No source files, no review threads, and no formal price / renderer / raw warehouse / frontend scope. Owner current-main confirmation is still absent, but Deputy accepts current-main merge-tree plus allowed-docs evidence for gate routing. | 0 | Deputy Codex | Stop ordinary owner chase unless branch head changes. Deputy owns final clean-scope gate decision; do not merge/reject from Executive/Triage. Request Codex re-review only if the PR head changes. | MEDIUM / DEPUTY_DECISION_MADE | No | No unless branch changes or Codex review reports NEEDS_FIX / P1 / P2 |
| Output Documents | Issue #18 / PR #23 | Output Documents Builder | `output/renderer-static-guard-review-packet` | `a75e380` | `https://github.com/laibeoffer/laibe-mvp/pull/23` | `DEPUTY2_WORKFLOW_REPAIR_ASSIGNED / CURRENT_MAIN_SYNC_BLOCKED` | PR #23 had clean Codex result `4531569296` on head `a75e380`, but current main advanced again. Current Deputy patrol on `origin/main` `a215335` finds local `git merge-tree --write-tree origin/main refs/remotes/origin/pr/23` exits `1` with a content conflict in `docs/WORKSTREAM_BLACKBOARD.md`; available `refs/pull/23/merge` still targets an older base. No owner response or branch update was found after Executive reassignment recommendation comment `4531941113`; the old P2 review thread is outdated with a fix reply, but the branch remains sync-blocked. | Prior checks are stale against latest main. Re-sync latest `origin/main`, rerun renderer static guard / syntax / mismatch / fixture / invalid fixture / `.xlsx/.pdf` diff / `git diff --check`, and request Codex re-review if the head changes. | 4 | Deputy Codex-2 | Deputy Codex-2 repair assignment remains active: re-sync PR #23 against current main, preserve fail-closed P2 fix and patrol docs, resolve only PR #23 / Output Documents conflicts, rerun checks, and request Codex re-review if head changes. | MEDIUM / DEPUTY_DECISION_MADE | No | No unless new Codex review reports NEEDS_FIX / P1 / P2 or scope drifts |
