...
| Raw Candidate | Issue #17 / PR #26 | Raw Candidate Builder | `warehouse/raw-source-quality-scoring` | `7853fe7` | `https://github.com/laibeoffer/laibe-mvp/pull/26` | `CURRENT_MAIN_SIMULATION_PASS_MERGE_REF_STALE` | PR #26 previously had Executive validation `4531540239`, Codex clean result `4531555287`, and Deputy gate routing `4531573641`. Executive follow-up comment `4531733938` was posted after main advanced, but its conflict wording is now superseded: Deputy patrol rechecked `origin/main` `e655829` and `git merge-tree --write-tree origin/main origin/pr/26` exits `0`. GitHub API reports `mergeable=true` / `clean`, while `refs/pull/26/merge` still targets old base `0e8ab82`, so final-gate evidence is stale but no current-main content-conflict signal is present. | Prior candidate-only / forbidden formal-pricing checks are stale for final-gate purposes until latest current-main evidence is refreshed; rerun the R1.5 validation set and request Codex re-review only if the PR head changes. | 1 | Executive Officer + Raw Candidate Builder | Correct the owner chase wording: request current-main evidence refresh, not conflict repair. Keep final gate paused until mergeability / validation / Codex signal is fresh against current main. | MEDIUM | No | No unless new Codex review reports NEEDS_FIX / P1 / P2, formal-price risk, or scope drift |
