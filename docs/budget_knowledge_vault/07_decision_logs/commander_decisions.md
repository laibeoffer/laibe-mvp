...
| Issue #49 / PR #47 | Issue #49 asks the Budget Engine Entry & Picking Agent to post or mirror the active resolution report. PR #47 records docs-only `NO_ENTRY_FOUND_MINIMAL_DRY_RUN_ENTRY_REQUIRED` evidence and says Commander should decide whether to authorize scoped runtime implementation and placeholder-vs-reviewed deterministic dry-run amounts. | This is a conditional commander decision input, not a commander decision and not authorization for this vault to implement or decide pricing behavior. | Mirror the blocker evidence and pending decision signal only; do not create runtime code, formal prices, `PricingRule`, `BudgetEstimateLine`, or integration harness output. |
