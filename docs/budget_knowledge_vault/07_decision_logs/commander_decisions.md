...
| Issue #49 / PR #47 | Issue #49 now contains the active investigation report and records `Need Commander: Yes` because Commander authorization is needed before creating or restoring any Budget Engine runtime entry. PR #47 records the same docs-only `NO_ENTRY_FOUND_MINIMAL_DRY_RUN_ENTRY_REQUIRED` evidence and asks Commander to decide placeholder-vs-reviewed deterministic dry-run amounts. | This is a commander decision request from the blocker owner, not a commander decision and not authorization for this vault to implement or decide pricing behavior. | Mirror the blocker evidence and pending decision signal only; do not create runtime code, formal prices, `PricingRule`, `BudgetEstimateLine`, or integration harness output. |
