...
|---|---|---|---|
| Budget engine entry clarification | `integration_gap_register.md` records `BUDGET_ENGINE_ENTRY_BLOCKER`; Issue #49 / PR #47 record docs-only `NO_ENTRY_FOUND_MINIMAL_DRY_RUN_ENTRY_REQUIRED` evidence and ask for Commander authorization before runtime implementation. | Should a future scoped minimal dry-run entry consume `validateMethodSpecCatalog()` / `buildBudgetCatalogFromMethodSpec()` output as approved seed/rules only, and should it use zero-value placeholders or reviewed deterministic dry-run amounts? | Do not edit `MethodSpecCatalog`, budget engine code, `PricingRule`, `MaterialSpec`, or `LaborRule`; this vault cannot authorize runtime implementation or formal prices. |
