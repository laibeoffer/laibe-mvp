...
| Minimal dry-run entry authorization pending | MethodSpec / Budget Engine | Open | Issue #49 / PR #47 propose a minimal dry-run Budget Engine entry contract without implementing runtime code and say Commander should decide whether to authorize scoped runtime implementation and placeholder-vs-reviewed deterministic dry-run amounts. This vault must not make that decision. |
